...
do
  ...
  sshpass -p moxa ssh -o StrictHostKeyChecking=no -t moxa@10.0.$lineup.$i "echo moxa | sudo -S sed -i 's#value=\"\"#value=\"10.0.'$lineup'.'$((i+1))'\"#' /var/lib/tomcat8/conf/Catalina/localhost/feather$
  sshpass -p moxa ssh -o StrictHostKeyChecking=no -t moxa@10.0.$lineup.$i "echo moxa | sudo -S chmod a+x /home/moxa/modpoll"
